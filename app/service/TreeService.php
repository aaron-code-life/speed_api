...
class TreeService
{
    ...
    public static function getdata($data){
        ...
        $temp['name']      = $data['name'];
        if ($data['hidden'] > -1)$temp['hidden'] = (boolean)$data['hidden'];
        if ($data['always_show'] > -1)$temp['always_show'] = (boolean)$data['always_show'];
        if ($data['redirect'])$temp['redirect'] = $data['redirect'];
        $temp['meta']['title'] = $data['title'];
        $temp['meta']['icon']  = $data['icon'];
        if ($data['no_cache'] > -1)$temp['meta']['no_cache'] = (boolean)$data['no_cache'];
        return $temp;
    }
}